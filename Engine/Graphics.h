...
public:
	...
	void DrawClosedPolyline(const std::vector<Vec2>& verts, Vec2 translation, float scale_X, float scale_Y, float rotation, Color c) 
	{
		const auto Transform = [&](Vec2 v) -> Vec2 
		{ 
			const float cos_ = cos(rotation);
			const float sin_ = sin(rotation);
			const float vx_orig = v.x;
			const float vy_orig = v.y;
			v.x = cos_ * v.x - sin_ * v.y;
			v.y = sin_ * vx_orig + cos_ * vy_orig;
			v.x *= scale_X;
			v.y *= scale_Y;
			v += translation;
			return v;
